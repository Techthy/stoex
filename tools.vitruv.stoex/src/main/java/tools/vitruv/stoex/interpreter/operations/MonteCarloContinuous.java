...
public class MonteCarloContinuous {
    ...
    public BoxedCDF convertToCDF(BoxedPDF pdf) {
        ...
        for (ContinuousSample sample : cdf.getSamples()) {
            System.out
                    .println("CDF Sample - Value: " + sample.getValue() + ", Probability: " + sample.getProbability());
        }
     ...
     * function
     * where each sample's probability is the area up to and including that value).
     * This means the CDF at each sample is the sum of all probabilities for values
     * <= sample.getValue().
     */
    public BoxedCDF convertToCDFUpperRight(BoxedPDF pdf) {
        ...
        double epsilon = 0.02;
        if (Math.abs(cumulativeProbability - 1.0) > epsilon) {
            throw new IllegalArgumentException("Invalid BoxedPDF: CDF does not sum to 1.");
        }
     ...
     * the PDF
     * defines the upper right corner of a rectangle (step function interpretation).
    ...
    public BoxedPDF monteCarloEstimation(BoxedPDF left, BoxedPDF right) {
        ...
        // Normalize BoxedPDF
        double totalProbability = result.getSamples().stream().mapToDouble(ContinuousSample::getProbability).sum();
    ...
    // Prints PDF
    /**
     * Prints a histogram for a BoxedPDF, assuming each sample represents the upper
     * right corner of a rectangle (step function).
     * The probability for each bin is taken directly from the sample's probability
    ...
    public void printHistogram(BoxedPDF pdf, int bins) {
        ...
        for (int bin = 0; bin < bins; bin++) {
            ...
            while (sampleIdx < samples.size() && samples.get(sampleIdx).getValue() <= upperEdge) {
